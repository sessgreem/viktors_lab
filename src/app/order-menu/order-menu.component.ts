...
export class OrderMenuComponent implements OnInit {
  ...
  render(event) {
    ...
    if(this.desiredRank < this.currentRank || this.desiredRank == this.currentRank && this.desiredDiv <= this.currentDiv){
      // if(this.desiredRank == this.currentRank)
      this.totalDivisions = Number.NaN;
      this.chargeAmount = Number.NaN;
    } else {
      this.totalDivisions = ((4 * this.leagues[this.desiredRank].id) + this.divisions[this.desiredDiv].id) -
      ((4 * this.leagues[this.currentRank].id) + this.divisions[this.currentDiv].id);
 ...
 createOrder(){
  //  console.log(this.chargeAmount);
   ...
   this.orderService.setOrder(this.object).then(succ => console.log("successful")).catch(err => console.log(err));
 }
}
